...
const EstimatedEarningsCard = React.memo(({ origin, destination, numberOfBags, estimatedDistance, estimatedEarnings }) => {
  ...
  const showCalculating = (origin && destination) && (!isValidNumber(estimatedDistance) || !isValidNumber(estimatedEarnings));
  const canDisplay = origin && destination && isValidNumber(estimatedDistance) && isValidNumber(estimatedEarnings);
