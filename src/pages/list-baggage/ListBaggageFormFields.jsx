...
import { CalendarPlus as CalendarIcon, PackagePlus, Info, Ruler, Weight, User, Phone, ShieldAlert } from 'lucide-react';
...
const ListBaggageFormFields = React.memo(({ form, isLoading }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <Label htmlFor="bag_weight_kg" className="font-semibold dark:text-slate-200">Weight Per Bag (kg)</Label>
          <div className="relative">
            ...
            <Controller
              ...
              rules={{
                ...
                validate: (v) => {
                  const n = Number(v);
                  if (!Number.isFinite(n) || n <= 0) return "Enter a valid weight.";
                  if (n > MAX_BAGGAGE_WEIGHT_PER_BAG) return `Max ${MAX_BAGGAGE_WEIGHT_PER_BAG}kg per bag.`;
                  return true;
                }
              }}
              render={({ field }) => (
          ...
          {errors.bag_weight_kg && <p className="text-xs text-red-500 mt-1">{errors.bag_weight_kg.message}</p>}
        </div>

        <div>
          <Label className="font-semibold dark:text-slate-200">Dimensions Per Bag (cm)</Label>
          ...
          <div className="grid grid-cols-3 gap-3 mt-1">
            <div>{errors.bag_length_cm && <p className="text-xs text-red-500">{errors.bag_length_cm.message}</p>}</div>
            <div>{errors.bag_width_cm && <p className="text-xs text-red-500">{errors.bag_width_cm.message}</p>}</div>
            <div>{errors.bag_height_cm && <p className="text-xs text-red-500">{errors.bag_height_cm.message}</p>}</div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <Label htmlFor="recipient_name" className="font-semibold dark:text-slate-200">Recipient Name</Label>
            <Controller
              name="recipient_name"
              control={control}
          ...
          <div>
            <Label htmlFor="recipient_contact" className="font-semibold dark:text-slate-200">Recipient Contact</Label>
            <div className="relative">
            ...
            {errors.recipient_contact && <p className="text-xs text-red-500 mt-1">{errors.recipient_contact.message}</p>}
          </div>
        </div>
      ...
      <div className="rounded-lg border border-border/50 p-4 md:p-5 bg-muted/30 dark:bg-slate-800/40">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="font-semibold dark:text-slate-200">Dangerous Goods Declaration</p>
        </div>

        <Controller
          name="dangerous_goods_declaration"
          control={control}
          rules={{ validate: (v) => v === true || "You must confirm this declaration." }}
          render={({ field }) => (
        ...
        {errors.dangerous_goods_declaration && (
          <p className="text-xs text-red-500 mt-2">{errors.dangerous_goods_declaration.message}</p>
        )}
      </div>
    </div>  
  );
