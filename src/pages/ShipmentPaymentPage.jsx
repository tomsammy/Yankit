...
const ShipmentPaymentPage = () => {
    ...
    const handleGeneratePaymentLink = async () => {
        ...
        try {
            ...
            const { data: functionData, error: functionError } = await supabase.functions.invoke('create-stripe-checkout-session', {
                body: {
            ...
            if (functionData.sessionId) {
                ...
                if (updateError) {
                    toast({ variant: "destructive", title: "Status Update Failed", description: "Could not update shipment status, but your payment will proceed." });
                } else {
                    toast({ title: "Success!", description: "Redirecting to Stripe checkout..." });
                }
            ...
            toast({ variant: "destructive", title: "Payment Initiation Failed", description: error.message });
        } finally {
                ...
                <CardContent className="p-6 space-y-6">
                        ...
                        {paymentLink ? ( // This `paymentLink` state is not used for Stripe direct redirects, but kept for conditional rendering if needed
                            <div className="space-y-4">
                             ...
                             <Button onClick={handleGeneratePaymentLink} className="w-full text-lg py-6 bg-gradient-to-r from-primary to-blue-600 hover:opacity-90" disabled={isGeneratingLink}>
                                ...
                                {isGeneratingLink ? 'Initiating Payment...' : 'Proceed to Payment'}
                            </Button>
                        )}
                    </div>
                 ...
                 <CardFooter className="p-6 border-t dark:border-slate-700/50">
                    <p className="text-xs text-muted-foreground dark:text-slate-400">
                        You will be redirected to Stripe's secure payment page to complete your transaction. Yankit holds the funds until a traveler is found and the shipment is completed.
                    </p>
                </CardFooter>
            </Card>
        </motion.div>
