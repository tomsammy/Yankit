...
const EditShipmentPage = () => {
  ...
  const findAirportByLabel = useCallback(
    (label) =>
      globalAirportsList.find((a) => a.label === label) || {
        label,
        value: label,
      },
    [],
  ...
  const save = async (e) => {
    ...
    try {
      ...
      if (!Number.isFinite(bags) || bags < 1)
        throw new Error("Number of bags is invalid");
          ...
          <form onSubmit={save} className="space-y-5">
                ...
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      ...
                      !data.departure_date && "text-muted-foreground",
                    )}
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {data.departure_date
                      ? format(data.departure_date, "PPP")
                      : "Pick a date"}
                  </Button>
                </PopoverTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => update("number_of_bags", v)}
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <Input
                  placeholder="Length"
                  value={data.bag_length_cm}
                  onChange={(e) => update("bag_length_cm", e.target.value)}
                />
                <Input
                  placeholder="Width"
                  value={data.bag_width_cm}
                  onChange={(e) => update("bag_width_cm", e.target.value)}
                />
                <Input
                  placeholder="Height"
                  value={data.bag_height_cm}
                  onChange={(e) => update("bag_height_cm", e.target.value)}
                />
              </div>
            </div>
            ...
            <div className="border rounded-lg p-4 space-y-3">
              ...
              <div className="flex items-start gap-3">
                <Checkbox
                  ...
                  onCheckedChange={(v) =>
                    update("dangerous_goods_declaration", v === true)
                  }
                />
