...
const ContractControls = ({
  ...
  const handleUpdateContractStatus = async (
    ...
    if (!contract?.id) {
      toast({
        title: "Error",
        description: "No active contract to update.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleCreateContract = async () => {
    if (!listingId || !currentUserId || !travelerUserId || !senderUserId) {
      toast({
        title: "Error",
        description: "Missing information to create contract.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Contract initiated! Awaiting confirmation.",
        className: "bg-green-500 text-white",
      });
      if (onSendMessage) {
        onSendMessage(
          `System: Contract initiated by ${isSender ? "Sender" : "Traveler"}. Awaiting confirmation.`,
        );
      }
    } catch (error) {
    ...
    return (
      <div className="p-3 border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-900/50">
        ...
        >
          ...
          {isLoading ? "Initiating..." : "Initiate Shipment Contract"}
        </Button>
      </div>
    );
  }
    ...
    return (
      <div className="p-3 border-b dark:border-slate-700 bg-slate-100 dark:bg-slate-900/50 text-center">
        <p className="text-sm text-muted-foreground dark:text-slate-400 flex items-center justify-center">
          <Info size={16} className="mr-2" /> Waiting for Sender to initiate the
          contract.
        </p>
      </div>
    );
      ...
      case "Pending Confirmation":
        return (
          <Button
            onClick={() =>
              handleUpdateContractStatus(
                "Awaiting Pickup",
                "Contract confirmed! Ready for item pickup.",
                `System: Traveler confirmed the contract. Ready for item pickup.`,
              )
            }
            disabled={isLoading}
          ...
          >
            ...
            {isLoading ? "Confirming..." : "Confirm Contract"}
          </Button>
        );
        ...
        return (
          <Button
            onClick={() =>
              handleUpdateContractStatus(
                "In Transit",
                "Item picked up and in transit.",
                `System: Traveler marked item as picked up and in transit.`,
              )
            }
            disabled={isLoading}
          ...
          >
            ...
            {isLoading ? "Updating..." : "Mark as In Transit"}
          </Button>
        );
      case "Delivered":
        return (
          <div className="flex items-center justify-center text-green-600 dark:text-green-400">
            <CheckCircle size={18} className="mr-2" /> Item delivered. Waiting
            for Sender to complete.
          </div>
        );
      case "Completed":
        return (
      ...
      case "Pending Confirmation":
        return (
          <div className="flex items-center justify-center text-orange-500 dark:text-orange-400">
            <Info size={18} className="mr-2" /> Awaiting Traveler's
            confirmation.
          </div>
        );
      case "In Transit":
        return (
          <AlertDialog>
            <AlertDialogTrigger asChild>
              ...
              >
                <CheckCircle size={18} className="mr-2" />{" "}
                {isLoading ? "Processing..." : "Mark as Delivered & Complete"}
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  By confirming, you acknowledge that you have received the
                  item(s) and the transaction will be marked as completed. This
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  onClick={() =>
                    handleUpdateContractStatus(
                      "Completed",
                      "Shipment marked as delivered and completed!",
                      `System: Sender confirmed delivery. Transaction completed.`,
                    )
                  }
        ...
        return (
          <AlertDialog>
            <AlertDialogTrigger asChild>
              ...
              >
                <CheckCircle size={18} className="mr-2" />{" "}
                {isLoading ? "Processing..." : "Confirm Receipt & Complete"}
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Confirming receipt will complete the transaction. This action
                  cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  onClick={() =>
                    handleUpdateContractStatus(
                      "Completed",
                      "Shipment receipt confirmed and completed!",
                      `System: Sender confirmed receipt. Transaction completed.`,
                    )
                  }
                  className="bg-green-500 hover:bg-green-600"
                >
              ...
              <AlertDialogAction
                onClick={() =>
                  handleUpdateContractStatus(
                    "Cancelled",
                    "Contract cancelled.",
                    `System: Contract cancelled by ${isTraveler ? "Traveler" : "Sender"}.`,
                  )
                }
                ...
                contract.status === "In Transit"
                  ...
                  : contract.status === "Delivered"
                    ...
                    : contract.status === "Completed"
                      ...
                      : contract.status === "Awaiting Pickup"
