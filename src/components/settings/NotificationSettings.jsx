...
const NotificationSettings = () => {
  ...
  if (!preferences) {
    return (
      <p className="text-muted-foreground text-center">
        Notification settings are not available at the moment.
      </p>
    );
  }
  ...
  const preferenceItems = [
    ...
    {
      id: "shipment_update_email",
      label: "Shipment Updates",
      description: "Get notified about the status of your shipments.",
      key: "shipment_update_email",
    },
    {
  ...
  return (
    <div className="space-y-6">
      {preferenceItems.map((item) => (
        <div
          key={item.id}
          className="flex items-center justify-between p-4 rounded-lg bg-muted/50 dark:bg-slate-700/30"
        >
          <div>
            <Label
              htmlFor={item.id}
              className="font-semibold text-foreground dark:text-slate-200"
            >
              {item.label}
            </Label>
