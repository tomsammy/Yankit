...
const MobileNavItem = ({ path, name, icon: Icon, onClick }) => (
  <SheetClose asChild>
    <NavLink
      ...
      onClick={onClick}
      className={({ isActive }) =>
        `flex items-center px-4 py-3 rounded-lg text-base font-medium transition-all duration-200 ease-in-out group ${
          isActive
    ...
    >
      {({ isActive }) => (
        <div
          className={`flex items-center p-2 rounded ${isActive ? "bg-blue-50" : "bg-white"}`}
        >
          <Icon
            className={`w-6 h-6 mr-3 transition-colors duration-200 ${isActive ? "text-primary dark:text-blue-400" : "text-slate-500 dark:text-slate-400 group-hover:text-primary dark:group-hover:text-blue-400"}`}
          />
          <span className="flex-grow">{name}</span>
          {isActive && (
            <motion.div
              layoutId="activeMobileLinkIndicator"
              className="w-1 h-6 bg-primary dark:bg-blue-500 rounded-full"
            />
          )}
        </div>
      )}
