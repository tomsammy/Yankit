...
const defaultSteps = [
  ...
  },
  {
    ...
    description:
      "Travellers list their origin/destination. Senders search for a match.",
  },
  {
    icon: <Search />,
    title: "Match & Notify",
    description:
      "Our algorithm matches the pair and notifies the sender about the available slot.",
  },
  {
    icon: <ClipboardCheck />,
    title: "Pay & Declare",
    description:
      "Sender accepts, pays, provides recipient details, and signs the dangerous goods declaration.",
  },
  {
    icon: <MapPin />,
  ...
  },
  {
    ...
    description:
      "Traveller delivers to the recipient, and the transaction is completed.",
    isConfirmation: true,
  },
];
    ...
    <section className="py-16 md:py-24 bg-white dark:bg-slate-900">
      <div className="container mx-auto px-4">
        ...
        >
          {title && (
            <h2
              className="text-3xl md:text-4xl font-extrabold text-foreground dark:text-white mb-3"
              dangerouslySetInnerHTML={{ __html: title }}
            />
          )}
          {subtitle && (
            <p
              className="text-lg text-muted-foreground dark:text-slate-300 max-w-2xl mx-auto"
          ...
          {steps.map((step, index) => (
            ...
            >
              <div className="flex items-center mb-4">
                <div
                  className={`p-3 rounded-full mr-4 bg-gradient-to-br ${step.isConfirmation ? "from-green-400/20 to-emerald-500/20" : "from-primary/20 to-blue-500/20"}`}
                >
                  {React.cloneElement(step.icon, {
                    className: `${step.isConfirmation ? "text-green-500" : "text-primary"} w-6 h-6`,
                  })}
                </div>
              ...
              <p
                className="text-sm text-muted-foreground dark:text-slate-300 leading-relaxed"
